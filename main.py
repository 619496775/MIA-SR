...
if __name__ == '__main__':
    ...
    # dataset = data_partition('raw_jdata/user_action_part')
    dataset = data_partition('taobao/user_item')
    ...
    # ------------------------------ 添加category
    # dataset1 = data_partition('raw_jdata/user_cate_part++')
    dataset1 = data_partition('taobao/user_cat')
    [user_train1, user_valid1, user_test1, usernum1, itemnum1] = dataset1
    sampler1 = WarpSampler(user_train1, usernum1, itemnum1, batch_size=args.batch_size, maxlen=args.maxlen, n_workers=2)

    # dataset2 = data_partition('raw_jdata/user_brand_part++')
    dataset2 = data_partition('taobao/user_brand')
    [user_train2, user_valid2, user_test2, usernum2, itemnum2] = dataset2
    sampler2 = WarpSampler(user_train2, usernum2, itemnum2, batch_size=args.batch_size, maxlen=args.maxlen, n_workers=2)

    # dataset3 = data_partition('raw_jdata/user_shop_part++')
    dataset3 = data_partition('taobao/user_seller')
    [user_train3, user_valid3, user_test3, usernum3, itemnum3] = dataset3
    sampler3 = WarpSampler(user_train3, usernum3, itemnum3, batch_size=args.batch_size, maxlen=args.maxlen, n_workers=2)
    # ------------------------------

    f = open(os.path.join(args.dataset + '_' + args.train_dir, '2022.5.29_maxlen=400'), 'w')
    ...
    for epoch in range(epoch_start_idx, args.num_epochs + 1):
        ...
        for step in range(
            ...
            u3, seq3, pos3, neg3 = np.array(u3), np.array(seq3), np.array(
                pos3), np.array(neg3)
            # ---------------
            pos_logits, neg_logits, pos_logits1, neg_logits1, pos_logits2, neg_logits2, pos_logits3, neg_logits3 = model(u, seq, pos, neg, u1, seq1, pos1, neg1, u2, seq2, pos2, neg2, u3, seq3, pos3, neg3)
            ...
            print("loss in epoch {} iteration {}: {}".format(
                epoch, step,
                loss1.item()))  # expected 0.4~0.6 after init few epochs
