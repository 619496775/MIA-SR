'''
Author: your name
Date: 2021-10-09 12:16:48
LastEditTime: 2021-11-08 19:04:53
LastEditors: Please set LastEditors
Description: In User Settings Edit
FilePath: \SASRec.pytorch-master\model1.py
'''
import numpy as np
import torch
import dataloader
...
dataset = dataloader.Loader(path="ml-1m-new/")
...
class SASRec(torch.nn.Module):
    def __init__(self, user_num, item_num, args):
        ...
        self.item_emb = torch.nn.Embedding(9716,
                                           args.hidden_units,
                                           padding_idx=0)
        self.item_emb.weight.data[1:].copy_(
            torch.cat((Recmodel.computer()[0], Recmodel.computer()[1]),
                      0))  # attr
        # self.item_emb.weight.data[1:].copy_(Recmodel.computer()[1])
        self.pos_emb = torch.nn.Embedding(args.maxlen,
                                          args.hidden_units)  # TO IMPROVE
        ...
        for _ in range(args.num_blocks):
            new_attn_layernorm = torch.nn.LayerNorm(args.hidden_units,
                                                    eps=1e-8)
        ...
        self.attn1 = AttentionLayer(args.hidden_units, attn_size=32,dropout=0.2 )
    ...
    def log2feats(self, log_seqs):
        ...
        positions = np.tile(np.array(range(log_seqs.shape[1])),
                            [log_seqs.shape[0], 1])  # 沿y轴复制log_seqs.shape[0]
    ...
    def forward(self, user_ids, log_seqs, pos_seqs, neg_seqs, user_ids1,
        ...
        log_feats1 = self.log2feats(log_seqs1)
        ...
        # print(log_feats1.shape)
        # print(pos_embs1.shape)
        pos_logits1 = (log_featsb * pos_embs1).sum(dim=-1)
        neg_logits1 = (log_featsb * neg_embs1).sum(dim=-1)

        return pos_logits, neg_logits, pos_logits1, neg_logits1
    ...
    #     log_feats = torch.cat((log_feats0, log_feats1),
    #                           1)
    ...
    #     # pos_embs2 = torch.cat((pos_embs2, pos_embs2, pos_embs2, pos_embs2), 1)
    #     # neg_embs2 = torch.cat((neg_embs2, neg_embs2, neg_embs2, neg_embs2), 1)

    #     # pos_logits2 = (log_feats * pos_embs2).sum(dim=-1)
    #     # neg_logits2 = (log_feats * neg_embs2).sum(dim=-1)
    ...
    #     # pos_embs3 = torch.cat((pos_embs3, pos_embs3, pos_embs3, pos_embs3), 1)
    #     # neg_embs3 = torch.cat((neg_embs3, neg_embs3, neg_embs3, neg_embs3), 1)

    #     # pos_logits3 = (log_feats * pos_embs3).sum(dim=-1)
    #     # neg_logits3 = (log_feats * neg_embs3).sum(dim=-1)
